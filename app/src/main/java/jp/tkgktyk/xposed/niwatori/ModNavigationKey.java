                    ...
                    private void modifyRecentsKey(final Object phoneStatusBar, View navigationBarView) {
                        ...
                        final GestureDetector gestureDetector = new GestureDetector(
                            ...
                            public boolean onSingleTapConfirmed(MotionEvent e) {
                                try {
                                    NFW.Settings settings = newSettings(mPrefs);
                                    if (NFW.isDefaultAction(settings.actionWhenTapOnRecents)) {
                                        clickListener.onClick(recentsButton);
                                    } else {
                                        NFW.performAction(recentsButton.getContext(),
                                                settings.actionWhenTapOnRecents);
                                    }
                                } catch (Throwable t) {
                                    logE(t);
                                }
                            ...
                            public void onLongPress(MotionEvent e) {
                                try {
                                    NFW.Settings settings = newSettings(mPrefs);
                                    if (NFW.isDefaultAction(settings.actionWhenLongPressOnRecents)) {
                                        ...
                                        NFW.performAction(recentsButton.getContext(),
                                                settings.actionWhenLongPressOnRecents);
                                    }
                                } catch (Throwable t) {
                                    logE(t);
                                }
                            ...
                            public boolean onDoubleTap(MotionEvent e) {
                                try {
                                    NFW.Settings settings = newSettings(mPrefs);
                                    if (NFW.isDefaultAction(settings.actionWhenDoubleTapOnRecents)) {
                                        if (!NFW.isDefaultAction(settings.actionWhenTapOnRecents)) {
                                            clickListener.onClick(recentsButton);
                                        } else {
                                            return false;
                                        ...
                                        NFW.performAction(recentsButton.getContext(),
                                                settings.actionWhenDoubleTapOnRecents);
                                    }
                                } catch (Throwable t) {
                                    logE(t);
                                }
